...
export class TypeActionComponent implements OnInit {
  ...
  trackId(index: number, item: ITypeAction): number {
    return item.id!;
  }

  delete(typeAction: ITypeAction): void {
    const modalRef = this.modalService.open(TypeActionDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ITypeAction[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-action'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
