...
export class UserExtraService {
  ...
  protected convertDateFromClient(userExtra: IUserExtra): IUserExtra {
    return Object.assign({}, userExtra, {
      dateDeNaissance: userExtra.dateDeNaissance?.isValid() ? userExtra.dateDeNaissance.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((userExtra: IUserExtra) => {
        userExtra.dateDeNaissance = userExtra.dateDeNaissance ? dayjs(userExtra.dateDeNaissance) : undefined;
      });
    }
    return res;
  }
